...
namespace MyEshop.Areas.Admin.Controllers
{
    public class SlidersController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "SliderID,Title,Url,ImageName,StartDate,EndDate,IsActive")] Slider slider, HttpPostedFile imgUp)
        {
            if (ModelState.IsValid)
            {
                if (imgUp != null)
                {
                    System.IO.File.Delete("/Images/Slider/" + slider.ImageName);
                    slider.ImageName = Guid.NewGuid().ToString() + System.IO.Path.GetExtension(imgUp.FileName);
                    imgUp.SaveAs(Server.MapPath("/Images/Slider/" + slider.ImageName));
                }
